...
/**
 * Simple system for testing retrieval based on a deep-learned embeddings of a
 * corpus of documents
 * and queries that have already been encoded. Assumes that document and queries
 * are files that contain
 * precomputed deep embeddings rather than the original text. This retriever
 * simply computes similarity of
 * an embedded query to embeddings of all the documents (using either Euclidian
 * distance or cosine
...
public class DeepRetriever {
    ...
    /**
     * The directory from which the documents come. This directory should contain a
     * file for
     * each document that contains a dense vector (list of real-values separated by
     * spaces)
     ...
     *                  Each file should contain a space-separated list of real
     *                  values
     * @param useCosine Flag to indicate use of cosine rather than Euclidian
    ...
    /**
     * Perform ranked retrieval on an input query encoded as a
     * DeepDocumentReference.
     */
    ...
    /**
     * Intended for a simple test run on a small corpus and optional test query
     * file.
     * Load a directory of files and then print them, can also include an extra
     * test file as a 2nd file arg if you want to also test a sample query vector
     * stored in a file,
     * in which case, you can include a "-cosine" flag to use cosine similarity
